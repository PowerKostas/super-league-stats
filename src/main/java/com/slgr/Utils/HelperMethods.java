...
public class HelperMethods {
    public static void addRowSorted(VBox tempVBox, HBox row, int orderChoice, int orderType) {
        // Does ordered insertion so when adding a row in the VBox, the table still appears in orderChoice order
        ArrayList<Integer> keys = (ArrayList<Integer>) row.getUserData();
        int data = keys.get(orderChoice);
        ...
        for (int i = 0; i < tempVBox.getChildren().size(); i += 1) {
            ...
            int key = keys.get(orderChoice);

            if (data * orderType < key * orderType) { // orderType: 1 = Ascending Order | -1 = Descending Order
